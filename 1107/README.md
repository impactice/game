...
기관포 2줄 - 평행 2발 사격
기관포 3줄 - 평행 3발 사격 (1번 구매 후)
미사일 해금 - 미사일 기능 활성화
미사일 2발 - 미사일 2발 발사 (3번 구매 후)
연사력 강화 - 발사 속도 증가 (4단계)
데미지 강화 - 데미지 증가 및 탄환 스킨 변경 (4단계)
체력 강화 - 최대 체력 증가 (4단계)
...
carrier_event.js: 캐리어 이벤트 미니게임 미구현
game.js 인코딩: 일부 한글 주석 깨짐 현상
이미지 로딩: 일부 이미지 경로 확인 필요
...
프로젝트 클론 또는 다운로드
웹 서버로 실행 (로컬 파일로는 일부 기능 제한)
intro.html 또는 index.html 열기

bash# 간단한 로컬 서버 실행 예시
